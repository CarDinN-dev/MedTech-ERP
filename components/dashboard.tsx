...
const pipeline = [
  { stage: "Lead", count: 28, value: "QAR 1.12M", width: "100%", color: "bg-blue-500" },
  { stage: "Qualified", count: 19, value: "QAR 1.48M", width: "82%", color: "bg-blue-600" },
  { stage: "Proposal", count: 14, value: "QAR 1.36M", width: "66%", color: "bg-[var(--corporate-navy)]" },
  { stage: "Negotiation", count: 8, value: "QAR 860K", width: "46%", color: "bg-amber-500" }
];
...
export function Dashboard() {
  ...
  return <div className="mx-auto max-w-[1600px] space-y-5 p-4 md:p-7">
    <section className="rounded-xl border border-[var(--line-soft)] bg-[var(--panel)] p-5 shadow-soft md:p-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="max-w-3xl">
          <div className="mb-2 flex items-center gap-2 text-xs font-bold uppercase tracking-[.14em] text-[var(--brand-red)]"><ShieldAlert className="h-4 w-4" /> Executive overview</div>
          <h1 className="text-2xl font-bold tracking-tight text-[var(--text)] md:text-[30px]">Good morning, {firstName}</h1>
          <p className="mt-2 text-sm leading-6 text-[var(--muted)]">MedTech is operating with strong revenue momentum, controlled approval queues, and active stock/service follow-up items requiring management attention.</p>
        </div>
    ...
    <section className="grid gap-5 xl:grid-cols-[1.55fr_1fr]">
        ...
        <div className="grid gap-5 lg:grid-cols-[1.15fr_.85fr]">
          <div>
            <div className="mb-4 flex flex-wrap items-end gap-3">
              ...
              <span className="rounded-full bg-emerald-50 px-2.5 py-1 text-[11px] font-bold text-emerald-700 dark:bg-emerald-950/45 dark:text-emerald-300">+15.2% YTD</span>
      ...
      <Panel title="Pending Approvals" subtitle={`${approvalTasks.length || 7} requests need attention`} action="Open queue" href="/approvals">
        <div className="space-y-2">{approvalRows.map((row, index) => <Link key={`${row.id}-${index}`} href="/approvals" className="flex items-center gap-3 rounded-xl border border-[var(--line-soft)] p-3 transition hover:bg-[var(--elevated)]"><StatusIcon tone="amber"><Clock3 className="h-4 w-4" /></StatusIcon><div className="min-w-0 flex-1"><div className="truncate text-xs font-bold">{row.id}</div><div className="truncate text-[11px] text-[var(--muted)]">{row.label}</div></div><div className="text-right"><div className="text-xs font-bold">{row.value}</div><div className="text-[10px] text-amber-600">{row.due}</div></div><ChevronRight className="h-4 w-4 text-[var(--muted)]" /></Link>)}</div>
      </Panel>
    </section>

    <section className="grid gap-5 xl:grid-cols-3">
      <Panel title="Warehouse Health" subtitle="Availability, low stock and expiry control" action="Inventory" href="/inventory">
        <div className="flex items-center gap-5"><StockDonut /><div className="flex-1 space-y-3 text-xs"><MetricLine label="Healthy stock" value="82%" color="bg-[var(--corporate-navy)]" /><MetricLine label="Low stock" value="23 SKUs" color="bg-amber-500" /><MetricLine label="Expiring stock" value="14 items" color="bg-rose-500" /></div></div>
        <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800 dark:border-amber-900 dark:bg-amber-950/40 dark:text-amber-300">23 SKUs are below minimum stock level. 14 lots need expiry review within 90 days.</div>
      </Panel>
      ...
      <Panel title="Workforce Attendance" subtitle="Today at 09:30" action="Open HR" href="/hr">
        ...
        <div className="space-y-3"><Progress label="Present" value="114" width="90.5%" color="bg-emerald-500" /><Progress label="On leave" value="8" width="6.3%" color="bg-blue-500" /><Progress label="Absent" value="4" width="3.2%" color="bg-rose-500" /></div>
      </Panel>

      <Panel title="Alerts" subtitle={`${alerts.length || 64} local alerts requiring review`} action="View alerts" href="/alerts">
        <div className="space-y-2">{(alerts.length ? alerts.slice(0, 4) : []).map(alert => <Link key={alert["Alert No"]} href={alertLink(alert)} className="flex gap-3 rounded-xl border border-[var(--line-soft)] p-3 transition hover:bg-[var(--elevated)]"><span className={cn("mt-1.5 h-2 w-2 shrink-0 rounded-full", alert.Priority === "Critical" ? "bg-[var(--brand-red)]" : "bg-amber-500")} /><span className="min-w-0 flex-1"><span className="block truncate text-xs font-bold">{alert["Alert Type"]}</span><span className="block truncate text-[11px] text-[var(--muted)]">{alert["Source Module"]} - {alert["Source Record"]}</span></span><span className="text-[10px] font-bold text-[var(--muted)]">{alert.Priority}</span></Link>)}
          {!alerts.length && <div className="rounded-xl border border-[var(--line-soft)] p-5 text-center text-xs text-[var(--muted)]">No active local alerts.</div>}
        </div>
      </Panel>
    </section>
    ...
    <section className="grid gap-5 xl:grid-cols-[1.15fr_.85fr]">
      ...
      <Panel title="My Tasks / Quick Actions" subtitle={`${tasks.length || 12} local work items and common actions`}>
        <div className="mb-4 space-y-2">{tasks.slice(0, 3).map(task => <Link key={task["Task No"]} href={task["Link/Open Record"]} className="flex items-center gap-3 rounded-xl border border-[var(--line-soft)] p-3 transition hover:bg-[var(--elevated)]"><StatusIcon tone={task.Priority === "Critical" ? "danger" : "amber"}><FileText className="h-4 w-4" /></StatusIcon><div className="min-w-0 flex-1"><div className="truncate text-xs font-bold">{task["Action Required"]}</div><div className="truncate text-[11px] text-[var(--muted)]">{task["Source Module"]} - {task["Source Record"]}</div></div><ChevronRight className="h-4 w-4 text-[var(--muted)]" /></Link>)}</div>
        <div className="grid grid-cols-2 gap-2">{quickActions.map(action => { const Icon = action.icon; return <Link key={action.label} href={action.href} className="group flex items-center gap-2 rounded-lg border border-[var(--line-soft)] p-2.5 text-xs font-bold transition hover:border-medtech-navy/25 hover:bg-[var(--navy-tint)] dark:hover:bg-[var(--elevated)]"><Icon className="h-4 w-4 text-[var(--corporate-navy)] dark:text-[var(--text-secondary)]" /><span className="truncate">{action.label}</span></Link>; })}</div>
      </Panel>
    </section>
  </div>;
}
...
function SummaryItem({ label, value }: { label: string; value: string }) {
  return <div><div className="text-[10px] font-bold uppercase tracking-[.14em] text-[var(--muted)]">{label}</div><div className="mt-1 font-semibold text-[var(--text)]">{value}</div></div>;
}
...
function KpiCard({ label, value, note, href, icon: Icon, tone }: { label: string; value: string; note: string; href: string; icon: typeof Users; tone: "navy" | "amber" | "blue" | "green" | "rose" }) {
  return <Link href={href} className="group rounded-xl border border-[var(--line-soft)] bg-[var(--panel)] p-4 shadow-soft transition hover:-translate-y-0.5 hover:border-medtech-navy/25">
...
function Panel({ title, subtitle, action, href = "#", children }: { title: string; subtitle?: string; action?: string; href?: string; children: React.ReactNode }) {
  return <section className="rounded-xl border border-[var(--line-soft)] bg-[var(--panel)] p-5 shadow-soft">
    <div className="mb-4 flex items-start justify-between gap-3"><div><h2 className="text-base font-bold text-[var(--text)]">{title}</h2>{subtitle && <p className="mt-1 text-[11px] text-[var(--muted)]">{subtitle}</p>}</div>{action && <Link href={href} className="flex shrink-0 items-center gap-1 text-[11px] font-bold text-[var(--brand-red)] hover:text-medtech-navy">{action}<ArrowRight className="h-3.5 w-3.5" /></Link>}</div>
    {children}
  </section>;
}
...
function StatusIcon({ tone, size = "md", children }: { tone: string; size?: "md" | "lg"; children: React.ReactNode }) {
  const tones: Record<string, string> = {
    navy: "bg-[var(--navy-tint)] text-medtech-navy dark:bg-[var(--elevated)] dark:text-[var(--text-secondary)]",
    amber: "bg-amber-50 text-amber-700 dark:bg-amber-950/45 dark:text-amber-300",
    blue: "bg-blue-50 text-blue-700 dark:bg-blue-950/45 dark:text-blue-300",
    green: "bg-emerald-50 text-emerald-700 dark:bg-emerald-950/45 dark:text-emerald-300",
...
function Progress({ label, value, width, color }: { label: string; value: string; width: string; color: string }) {
  return <div><div className="mb-1.5 flex text-[11px]"><span>{label}</span><span className="ml-auto font-bold tabular">{value}</span></div><div className="h-1.5 overflow-hidden rounded-full bg-[var(--elevated)]"><div className={cn("h-full rounded-full", color)} style={{ width }} /></div></div>;
}
...
function RevenueChart() {
  ...
  return <div className="h-[235px]"><svg viewBox={`0 0 ${width} ${height}`} className="h-full w-full" role="img" aria-label="Revenue performance chart">
    <defs><linearGradient id="execRevenueFill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#243168" stopOpacity="0.18" /><stop offset="100%" stopColor="#243168" stopOpacity="0" /></linearGradient></defs>
    ...
    <path d={revenuePath} fill="none" stroke="#243168" strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round" />
    {revenueData.map((row, index) => { const [x, y] = xy(row.revenue, index); return <g key={row.month}><circle cx={x} cy={y} r="4" fill="#243168" /><text x={x} y={height - 4} textAnchor="middle" fontSize="11" fill="#94a3b8">{row.month}</text></g>; })}
  </svg></div>;
}

function StockDonut() {
  return <div className="grid h-32 w-32 shrink-0 place-items-center rounded-full" style={{ background: "conic-gradient(#243168 0 82%, #f59e0b 82% 94%, #e11d48 94% 100%)" }}><div className="grid h-20 w-20 place-items-center rounded-full bg-[var(--panel)] text-center"><b className="text-lg">82%</b><span className="-mt-1 text-[10px] text-[var(--muted)]">healthy</span></div></div>;
}
