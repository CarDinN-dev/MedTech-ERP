  ...
  return <div className="mx-auto max-w-[1600px] p-4 md:p-7">
    ...
    <section className="overflow-hidden rounded-2xl border bg-[var(--panel)] shadow-soft">
      <div className="flex flex-wrap items-center gap-3 border-b px-5 py-3.5">
        <div className="relative min-w-[230px] flex-1 md:max-w-sm"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search alerts..." className="h-9 w-full rounded-lg border bg-[var(--panel)] pl-9 pr-3 text-sm outline-none focus:border-medtech-red" /></div>
      ...
      <div className="overflow-x-auto"><table className="w-full min-w-[1460px] text-left text-xs">
        ...
        <tbody className="divide-y">{filtered.map(alert => <tr key={alert["Alert No"]} className="hover:bg-slate-50/80 dark:hover:bg-slate-900/40">{alertColumns.map((column, index) => <td key={column} className={cn("px-4 py-3 align-top", index === 0 && "font-semibold")}>{column === "Status" || column === "Priority" ? <StatusBadge>{alert[column]}</StatusBadge> : alert[column]}</td>)}<td className="px-4 py-3"><Link href={alertLink(alert)} className="inline-flex items-center gap-1 font-semibold text-medtech-navy hover:text-medtech-deep focus-visible:outline-none focus-visible:shadow-focus">Open <ExternalLink className="h-3 w-3" /></Link></td><td className="px-4 py-3"><ActionMenu label={`Actions for ${alert["Alert No"]}`} actions={[
          { label: "Resolve alert", icon: <CheckCircle2 className="h-3.5 w-3.5" />, disabled: alert.Status === "Resolved", onClick: () => act("Resolved", resolveAlert, alert["Alert No"]) },
          { label: "Snooze 7 days", icon: <Clock3 className="h-3.5 w-3.5" />, disabled: alert.Status === "Resolved", onClick: () => act("Snoozed", (no, user) => snoozeAlert(no, 7, user), alert["Alert No"]) },
          { label: "Escalate alert", icon: <ArrowUpRight className="h-3.5 w-3.5" />, disabled: alert.Status === "Resolved", onClick: () => act("Escalated", escalateAlert, alert["Alert No"]) }
        ]} /></td></tr>)}</tbody>
      </table></div>
