  ...
  return <div className="grid min-h-[680px] xl:grid-cols-[1fr_380px]">
    <div className="min-w-0">
      <div className="flex flex-wrap items-center gap-3 border-b px-5 py-3.5">
        <div className="relative min-w-[220px] flex-1 md:max-w-sm"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search approvals..." className="h-9 w-full rounded-lg border bg-[var(--panel)] pl-9 pr-3 text-sm outline-none focus:border-teal-500" /></div>
    ...
    <aside className="border-t p-5 xl:border-l xl:border-t-0">
      {selected ? <div className="space-y-4">
        ...
        <textarea value={notes} onChange={event => setNotes(event.target.value)} placeholder="Decision notes..." className="min-h-24 w-full rounded-xl border bg-[var(--panel)] px-3 py-2 text-sm outline-none focus:border-teal-500" />
