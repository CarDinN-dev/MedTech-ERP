...
export function RecordModal({ open, title, columns, formColumns, selectOptions, defaultValues, fieldTypes, suggestions, record, approvalMode, onClose, onSave, onDelete, onDecision }: Props) {
  ...
  useEffect(() => { if (open) { setValues(Object.fromEntries(fields.map(column => [column, normalizeInitialValue(column, record?.[column] ?? resolvedDefaultValues[column] ?? defaultValue(column), resolvedFieldTypes)]))); setTab("details"); } }, [open, record, fields, resolvedDefaultValues, resolvedFieldTypes]);
  if (!open) return null;
  const submit = (event: React.FormEvent) => { event.preventDefault(); onSave(values); };
  const updateField = (column: string, value: string) => {
    setValues(current => {
      const exactSuggestion = resolvedSuggestions[column]?.find(option => option.value.toLowerCase() === value.toLowerCase());
      return { ...current, [column]: value, ...(exactSuggestion?.fill ?? {}) };
    });
  };
