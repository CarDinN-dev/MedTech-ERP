...
export function RecordModal({ open, title, columns, formColumns, selectOptions, defaultValues, fieldTypes, suggestions, record, approvalMode, onClose, onSave, onDelete, onDecision, deriveValues, preview }: Props) {
  ...
  return <div className="fixed inset-0 z-[90] flex items-center justify-center bg-slate-950/55 p-2 backdrop-blur-sm sm:p-4" onMouseDown={onClose}>
    <div role="dialog" aria-modal="true" aria-label={title} onMouseDown={event => event.stopPropagation()} className="max-h-[calc(100vh-1rem)] w-full max-w-4xl overflow-hidden rounded-2xl border bg-[var(--panel)] shadow-panel animate-in sm:max-h-[92vh]">
      <div className="flex items-center gap-3 border-b px-5 py-4">
        <div className="rounded-xl bg-[var(--navy-tint)] p-2.5 text-medtech-red dark:bg-[var(--elevated)]"><FileText className="h-5 w-5" /></div>
        ...
        <button type="button" aria-label="Close form" onClick={onClose} className="ml-auto rounded-lg p-2 text-slate-400 hover:bg-slate-100 focus-visible:outline-none focus-visible:shadow-focus dark:hover:bg-slate-800"><X className="h-5 w-5" /></button>
      </div>
      {record && <div className="flex gap-1 border-b px-5">
        <button type="button" onClick={() => setTab("details")} className={`flex items-center gap-2 px-3 py-3 text-xs font-semibold focus-visible:outline-none focus-visible:shadow-focus ${tab === "details" ? "border-b-2 border-medtech-red text-medtech-red" : "text-slate-400"}`}><FileText className="h-3.5 w-3.5" /> Details</button>
        <button type="button" onClick={() => setTab("activity")} className={`flex items-center gap-2 px-3 py-3 text-xs font-semibold focus-visible:outline-none focus-visible:shadow-focus ${tab === "activity" ? "border-b-2 border-medtech-red text-medtech-red" : "text-slate-400"}`}><History className="h-3.5 w-3.5" /> Activity</button>
      </div>}
      ...
      {tab === "details" ? <form onSubmit={submit}>
        <div className="max-h-[calc(100vh-13rem)] space-y-4 overflow-y-auto bg-[var(--elevated)]/35 p-4 sm:max-h-[60vh] sm:p-5">
          {error && <div role="alert" className="rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-xs font-medium text-rose-700">{error}</div>}
        ...
        <div className="sticky bottom-0 flex flex-wrap items-center gap-2 border-t bg-slate-50/95 px-4 py-4 backdrop-blur dark:bg-slate-900/90 sm:px-5">
      ...
      </form> : <div className="min-h-[320px] p-5">
        <div className="space-y-4 border-l-2 border-slate-100 pl-5 dark:border-slate-800">{[["Record opened", "You viewed this record just now"], ["Status updated", `Status is ${record?.Status ?? record?.Stage ?? "Active"}`], ["Record created", "Created from the MedTech client demo dataset"]].map(([name, note], index) => <div key={name} className="relative"><span className="absolute -left-[27px] top-1 h-3 w-3 rounded-full border-2 border-[var(--panel)] bg-medtech-red" /><div className="text-xs font-semibold">{name}</div><div className="mt-0.5 text-[11px] text-slate-400">{note}</div><div className="mt-1 text-[10px] text-slate-300">{index === 0 ? "Now" : index === 1 ? "2 hours ago" : "18 June 2026"}</div></div>)}</div>
        <div className="mt-8 flex gap-2 rounded-xl border p-3"><MessageSquare className="mt-1 h-4 w-4 text-slate-400" /><input aria-label="Internal note" className="flex-1 bg-transparent text-xs outline-none" placeholder="Add an internal note..." /><Button type="button">Add note</Button></div>
      </div>}
    </div>
  </div>;
}
...
function renderField(column: string, fields: string[], values: Record<string, string>, submitted: boolean, record: DemoRecord | null | undefined, title: string, selectOptions: Record<string, string[]>, fieldTypes: Record<string, RecordFieldType>, suggestions: Record<string, RecordFieldSuggestion[]>, updateField: (column: string, value: string) => void) {
  ...
  return <label key={column} className={fieldIndex === 0 || longField ? "sm:col-span-2" : ""}>
    <span className="mb-1.5 block text-[11px] font-semibold text-[var(--muted)]">{cleanLabel(column)}{column === "Password" && !record ? " (temporary)" : ""}</span>
    {useSuggestionSelect ? <select aria-label={column} value={values[column] ?? ""} onChange={event => updateField(column, event.target.value)} className="h-10 w-full rounded-xl border bg-[var(--panel)] px-3 text-sm outline-none focus:border-medtech-red">
    ...
    </select> : options ? <select aria-label={column} value={values[column] ?? ""} onChange={event => updateField(column, event.target.value)} className="h-10 w-full rounded-xl border bg-[var(--panel)] px-3 text-sm outline-none focus:border-medtech-red">
      {Array.from(new Set([values[column], ...options].filter(Boolean))).map(option => <option key={option}>{option}</option>)}
    </select> : longField ? <textarea required={required} aria-label={label} aria-invalid={Boolean(fieldError)} value={values[column] ?? ""} onChange={event => updateField(column, event.target.value)} className="min-h-24 w-full resize-y rounded-xl border bg-[var(--panel)] px-3 py-2 text-sm outline-none transition focus:border-medtech-red focus:ring-2 focus:ring-[var(--focus-ring)]" placeholder={placeholder(column, type)} /> : <>
      <input required={required} minLength={column === "Password" ? 8 : undefined} aria-label={label} aria-invalid={Boolean(fieldError)} type={type} list={columnSuggestions.length ? datalistId : undefined} autoComplete={column === "Password" ? "new-password" : undefined} value={values[column] ?? ""} onChange={event => updateField(column, event.target.value)} className="h-10 w-full rounded-xl border bg-[var(--panel)] px-3 text-sm outline-none transition focus:border-medtech-red focus:ring-2 focus:ring-[var(--focus-ring)]" placeholder={column === "Password" ? (record ? "Leave blank to keep current password" : "Minimum 8 characters") : placeholder(column, type)} />
    ...
    <p className={fieldError ? "mt-1 text-[11px] font-medium text-rose-600" : "mt-1 text-[11px] text-[var(--muted)]"}>{fieldError || helperText(column, type, required)}</p>
  </label>;
}
