...
export interface ListOptions {
  ...
  orderBy?: string; ascending?: boolean; filters?: Record<string, string | number | boolean>;
}
...
export class Repository<TCreate extends object, TUpdate extends object = Partial<TCreate>> {
  ...
  async list(options: ListOptions = {}) {
    ...
    let query = this.client.from(this.table).select("*", { count: "exact" }).is("deleted_at", null);
  ...
  async find(id: string) { const { data, error } = await this.client.from(this.table).select("*").eq("id", id).is("deleted_at", null).single(); if (error) throw new DataError(error.message, error.code); return data; }
